...
export default function ManagePrPo() {
  ...
  // ================= FETCH (PR / PO ONLY) =================
  const fetchPrPo = async () => {
    try {
      setLoad(true);
      const res = await ApiServices.GetAllPrPo();

      if (res?.data?.success) {
        // 🔥 HARD FILTER: ONLY PR / PO
        const prpoOnly = (res.data.data || []).filter(
          (u) => u.designation === "PR/PO"
        );
        setData(prpoOnly);
      } else {
        setData([]);
  ...
  function changeInactiveStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        ApiServices.ChangeStatusPrPo(payload)
          .then((res) => {
            Swal.fire({
