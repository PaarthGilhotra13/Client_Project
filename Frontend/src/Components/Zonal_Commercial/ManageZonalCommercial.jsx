...
export default function ManageZonalCommercial() {
  ...
  // ================= FETCH (ZONAL COMMERCIAL ONLY) =================
  const fetchZonalCommercial = async () => {
    try {
      setLoad(true);
      const res = await ApiServices.GetAllZonalCommercial();

      if (res?.data?.success) {
        // 🔥 HARD FILTER: ONLY ZONAL COMMERCIAL
        const zonalCommercialOnly = (res.data.data || []).filter(
          (u) => u.designation === "Zonal_Commercial"
        );
        setData(zonalCommercialOnly);
      } else {
        setData([]);
      }
    } catch (err) {
  ...
  function changeInactiveStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        ApiServices.ChangeStatusZonalCommercial(payload)
          .then((res) => {
            Swal.fire({
  ...
  return (
    <>
      <main className={`main ${modalOpen ? "blur-background" : ""}`} id="main">
        <PageTitle child="Manage Zonal Commercial Users" />
                ...
                {currentEmployees.length ? (
                  ...
                  <tr>
                    <td colSpan={9} className="text-center text-muted">
                      No Active Zonal Commercial User Found
                    </td>
                  </tr>
                )}
              </tbody>
