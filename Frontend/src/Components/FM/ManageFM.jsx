...
export default function ManageFM() {
  ...
  const fetchFMs = async () => {
    try {
      setLoad(true);
      const res = await ApiServices.GetAllFm();
  ...
  function changeInactiveStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        ApiServices.ChangeStatusFm(payload)
          .then((res) => {
            Swal.fire({
  ...
  return (
    <>
      <main className={`main ${modalOpen ? "blur-background" : ""}`} id="main">
        <PageTitle child="Manage Facility Managers" />
                ...
                {currentEmployees.length ? (
                  ...
                  <tr>
                    <td colSpan={9} className="text-center text-muted">
                      No Active Facility Manager Found
                    </td>
                  </tr>
                )}
              </tbody>
