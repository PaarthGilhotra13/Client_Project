...
export default function ManageMissingBridge() {
  ...
  // ================= FETCH (MISSING BRIDGE ONLY) =================
  const fetchMissingBridge = async () => {
    try {
      setLoad(true);
      const res = await ApiServices.GetAllMissingBridge();

      if (res?.data?.success) {
        // 🔥 HARD FILTER: ONLY MISSING BRIDGE
        const missingBridgeOnly = (res.data.data || []).filter(
          (u) => u.designation === "Missing_Bridge"
        );
        setData(missingBridgeOnly);
      } else {
        setData([]);
      }
    } catch (err) {
  ...
  function changeInactiveStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        ApiServices.ChangeStatusMissingBridge(payload)
          .then((res) => {
            Swal.fire({
  ...
  return (
    <>
      <main className={`main ${modalOpen ? "blur-background" : ""}`} id="main">
        <PageTitle child="Manage Missing Bridge Users" />
                ...
                {currentEmployees.length ? (
                  ...
                  <tr>
                    <td colSpan={9} className="text-center text-muted">
                      No Active Missing Bridge User Found
                    </td>
                  </tr>
                )}
              </tbody>
