...
export default function ManageProcurement() {
  ...
  // ================= FETCH (PROCUREMENT ONLY) =================
  const fetchProcurement = async () => {
    try {
      setLoad(true);
      const res = await ApiServices.GetAllProcurement();

      if (res?.data?.success) {
        // 🔥 HARD FILTER: ONLY PROCUREMENT
        const procurementOnly = (res.data.data || []).filter(
          (u) => u.designation === "Procurement"
        );
        setData(procurementOnly);
      } else {
        setData([]);
      }
    } catch (err) {
  ...
  function changeInactiveStatus(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        ApiServices.ChangeStatusProcurement(payload)
          .then((res) => {
            Swal.fire({
  ...
  return (
    <>
      <main className={`main ${modalOpen ? "blur-background" : ""}`} id="main">
        <PageTitle child="Manage Procurement" />
